...
import { TsConfig } from '../findTsConfig/types'
...
export type TsConfigPathsResolver = {
  readonly resolvePath: (moduleSpecifier: string) => Option<Uri>
...
export function createResolveTsConfigPaths({
  ...
  const resolvePath: (moduleSpecifier: string) => Option<Uri> = matchPath
    ? (specifier: string) =>
        pipe(
          matchPath(specifier, undefined, undefined, extensions),
          fromNullable,
          map((path) => Uri.wrap(`file://${path}`)),
        )
    : constant(none)
